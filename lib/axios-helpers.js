    ...
    rax: {
      ...
      shouldRetry: (err) => {
        ...
        if (raxcfg.currentRetryAttempt >= raxcfg.retry) {
          raxcfg.logger.error(
            `CorrelationId: ${err.config.correlationid}. Maximum retries reached.`,
            {
        ...
        if (err.message.includes('Request did not return JSON')) {
          logger.warn(
            `CorrelationId: ${err.config.correlationid}. Request did not return JSON. Retry attempt #${raxcfg.currentRetryAttempt}. Retrying request.`,
            {
              label: `shouldRetry-${err.config.axiosid}`,
            }
          );
        ...
        if (rax.shouldRetryRequest(err)) {
          raxcfg.logger.warn(
            `CorrelationId: ${err.config.correlationid}. Retry attempt #${raxcfg.currentRetryAttempt}. Retrying request.`,
            {
              label: `shouldRetry-${err.config.axiosid}`,
            }
          );
        ...
        raxcfg.logger.error(`CorrelationId: ${err.config.correlationid}. None retryable error.`, {
          label: `shouldRetry-${err.config.axiosid}`,
        });
        return false;
      },
